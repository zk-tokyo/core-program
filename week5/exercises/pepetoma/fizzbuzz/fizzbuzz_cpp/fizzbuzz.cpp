...
void FizzBuzz_8_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void FizzBuzz_8_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 1246;}

uint get_number_of_components() {return 271;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 691;}
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[8]); // line circom 41
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
...
uint csoffset = mySignalStart+510;
uint aux_dimensions[1] = {15};
for (uint i = 0; i < 15; i++) {
...
uint csoffset = mySignalStart+795;
uint aux_dimensions[1] = {15};
for (uint i = 0; i < 15; i++) {
...
uint csoffset = mySignalStart+1005;
uint aux_dimensions[1] = {15};
for (uint i = 0; i < 15; i++) {
std::string new_cmp_name = "muxMultipleOf3"+ctx->generate_position_array(aux_dimensions, 1, i);
Mux1_7_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+ i] = aux_cmp_num;
csoffset += 8 ;
aux_cmp_num += 2;
}
...
uint csoffset = mySignalStart+1125;
uint aux_dimensions[1] = {15};
for (uint i = 0; i < 15; i++) {
std::string new_cmp_name = "muxMultipleOf5"+ctx->generate_position_array(aux_dimensions, 1, i);
Mux1_7_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+ i] = aux_cmp_num;
csoffset += 8 ;
aux_cmp_num += 2;
...
uint csoffset = mySignalStart+885;
uint aux_dimensions[1] = {15};
for (uint i = 0; i < 15; i++) {
std::string new_cmp_name = "muxMultipleOf15"+ctx->generate_position_array(aux_dimensions, 1, i);
Mux1_7_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+ i] = aux_cmp_num;
csoffset += 8 ;
aux_cmp_num += 2;
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[7]); // line circom 55
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[2];
// load src
...
FizzBuzz_8_create(1,0,ctx,"main",0);
FizzBuzz_8_run(0,ctx);
}
